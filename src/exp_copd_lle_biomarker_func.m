function exp_copd_lle_biomarker_func(fev1_cov)
% Experiments on copd LLE phenotype & blood biomarker data.
out_path = '../outputs/copd_lle_biomarker';
if ~exist(out_path, 'dir')
    mkdir(out_path);
end

% set parameters
dataset.data_path = '../../data/copd_biomarker/';
dataset.phe_no = 2:101;
dataset.fev1 = fev1_cov;
% dataset.phe_no = 2:101; % LLE
% dataset.phe_no = 15; % FEV1
dataset.loader = @dataset_copd_lle_biomarker;
...
% save data and plots
save(fullfile(out_path,['pheno_' num2str(dataset.phe_no(1)) '_' num2str(dataset.phe_no(end)) ...
    '_' num2str(dataset.fev1) '_' num2str(pars.ratio) '_real.mat']),'p_val0','p_val','p_valSemi','Sta','StaSemi','nlSel','nlSelUp');
savefig(h,fullfile(out_path,['pheno_' num2str(dataset.phe_no(1)) '_' num2str(dataset.phe_no(end))...
    '_' num2str(dataset.fev1) '_' num2str(pars.ratio) '_real.fig']));
